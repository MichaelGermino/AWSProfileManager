...
3. **Exported `getEnterpriseHttpsAgent()`** — returns an `https.Agent` for callers that build their own. **The AWS SDK STSClient and the SAML axios client both consume this** (see `awsAuthService.ts`); without it they'd fail TLS verification behind a corporate proxy because `@smithy/node-http-handler` constructs a fresh agent that ignores `https.globalAgent`.
