...
- **SSO session** (`sso-sessions.json` in app data, NOT Keytar - see security-model.md for the 2560-byte Credential Manager limit): map of `sso:<sha256(startUrl|region) sliced to 32 hex>` to a safeStorage-encrypted base64 value whose plaintext is JSON `{ registration?: { clientId, clientSecret, clientSecretExpiresAt, redirectUri }, token?: { accessToken, refreshToken?, expiresAt }, identity? }`. Keyed by org, so N profiles share one entry. `clientSecretExpiresAt` is epoch **seconds** (from AWS); `token.expiresAt` is epoch **ms** (computed locally). `GetRoleCredentials` returns `expiration` in epoch **ms**.
